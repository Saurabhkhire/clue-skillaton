...
- **Remove number** when you tick or cross.
- **Single number in column**: If a player has only one number, tick it and cross that card under others.

## Deduction Rules

1. **Your cards**: ✓ for your hand, ✗ for the rest.
2. **When someone shows you a card**: Mark ✓ the one they have. Do nothing to the other two.
3. **When someone passes (couldn't disprove)**: Cross all 3 cards under that player. Same for each next person who passes.
4. **When you tick someone**: Rest of same row = ✗.
