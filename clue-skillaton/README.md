...
Run frontend and backend manually:
...
Each run starts a fresh game (no persisted game state).

**Kill port 5173** (if needed): `npx kill-port 5173` or `taskkill /PID <pid> /F` after `netstat -ano | findstr :5173`
